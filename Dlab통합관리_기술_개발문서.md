# Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) (DLab 통합 관리 프로그램) 기술 개발문서 (Technical Specification)
...
| 6 | 🔧 **양방향 동기화** — 대성전산 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 병행 운영 전제(단방향 이관 금지) | 6.1 · 13 |
...
| 19 | 🖼️ **질의응답 ON/OFF**(0710) — off(대면) 상담실 예약 스케줄 관리 + **영단어 시험(디랩 콘텐츠) 관리** | 8.13 |
...
### 1.1 프로젝트 설명
D.Lab(대성학력개발연구소) 전용 학원 관리 프로그램 신규 개발. 현재 대성전산이 개발·운영 중인 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))를 자체 시스템으로 대체한다. 재원생 앱과 **동일한 Spring Boot API 서버 + Supabase**를 공유한다.

### 1.2 운영 주체 및 관계사
| 주체 | 역할 | 비고 |
|---|---|---|
| 대성학력개발연구소(DSD) | D.Lab 사이트 운영, 발주처, **앱 스토어 계정 보유** | |
| 대성전산 | 현 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))·**디멤버**·수납 프로그램 개발·운영 | DB 포함 전체 소유 |
| 자이엘코리아 | Zyxel 방화벽 장비 관리 | 김상원 팀장 / 010-6412-8131 |
| 급식업체 | 급식 공급·PG 가맹점(벤더 명의) | 연락처·환불 처리 |
| 개발팀(숀아이비엔엘) | 신규 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))·앱 개발 | |

### 1.3 문서 관계
| 문서 | 대상 |
|---|---|
| `Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))_기술_개발문서.md` | 통합 관리 프로그램(웹, React) — **본 문서**, 공통 백엔드 명세 포함 |
...
### 2.1 프론트엔드 (Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 웹)
```
...
```
┌──────────────────────────────────────────────────┐
│  [Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 웹 (React)]     [재원생 앱 (Flutter)]         │
└───────────┬───────────────────────┬──────────────┘
            │ HTTPS / REST          │
            ▼                       ▼
┌──────────────────────────────────────────────────┐
...
```
com.dlab.Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))/
├── auth/           # JWT·RBAC
├── common/
...
- 🔧 **양방향 동기화:** 대성전산 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 계약 종료 전 **병행 운영** 가능 → 단방향 마이그레이션 가정 금지. `SyncAdapter`로 변경분 동기화(충돌 정책·정합성 배치).
...
```
연동 흐름(급식 결제): `PG Webhook → Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 저장 → 대성전산 수납 API 자동 반영`.
...
@Component class LunchVendorPgClient implements PaymentGatewayClient {
    // 벤더 명의. 환불은 벤더 직접 → Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))는 취소요청·기록만(requestRefund 미구현)
}
```
...
```
Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 발송 알림톡 템플릿(예): 출결(등원/지각/결석), 상벌점, 대기자 순번, 미납 안내, 급식 결제완료, 성적 업로드.
...
```
- 앱: 과목별 **추이 그래프**만. **상세 분석은 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))(담임 상담용)** 에서 표시(8.10 상담 리포트 연계).
...
```
- 학생 신청(사유·정기일정·방화벽 등) → `approver_type`에 따라 학부모 앱 또는 **선생님(Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))/관리자 앱)** 로 라우팅.
...
### 9.1 인증
```
Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 로그인(직원 ID+PW) → Supabase Auth → JWT(Access 1h / Refresh 7d)
...
| 6 | 🔧 **대성전산 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 계약 종료 시점(컷오버)** | 행정 | 🔴 |
| 7 | D.Lab 디자인 가이드 | 기획 | 🔴 |
| 8 | 교무업무 구분 항목 정리 | 운영 | 🔴 |
...
| 6 | 대성 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA)))))) 병행 운영 | **양방향 동기화 어댑터**(단방향 가정 금지), 컷오버 계획(D-8) |
...
*본 문서는 개발팀 내부 전용입니다. 확정 시 `Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 Dlab통합관리프로그램(구 DSA))))))_docx` 리포에 커밋하고 버전·요약을 갱신하세요. 재원생 앱은 `APP_기술_개발문서.md`를 참조합니다.*
