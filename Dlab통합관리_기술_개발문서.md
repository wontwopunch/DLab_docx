# Dlab통합관리프로그램(구 DSA) (DLab 통합 관리 프로그램) 기술 개발문서 (Technical Specification)

> **문서 유형**: 개발팀 내부 기술 문서
> **대상**: 백엔드·프론트엔드·DevOps 팀원, AI 코드 어시스턴트
> **버전**: v1.3 (2026-07-15) — v1.2 + 발주처·일정·인력 확정 반영
> **v1.3 변경**: ① ✅발주처=디랩(DSD는 관계사) ② ✅일정: ~~2개월~~ → 앱 개발완료 11/30·12월 심사·승인 후 사용자 QA (P3 납기 🟡계약 확정 필요) ③ ✅인력: BE 지유림(전기간)+이진규(11월 합류, P1 종료 후) / **웹 FE=서지원(PM 겸임)** / 메신저 React 연동 담당 유현님→서지원 ④ Phase에 목표 월 부여(12장)
...
| 6 | 🔧 **양방향 동기화** — 대성전산 Dlab통합관리프로그램(구 DSA) 병행 운영 전제(단방향 이관 금지) | 6.1 · 13 |
...
### 1.1 프로젝트 설명
D.Lab(대성학력개발연구소) 전용 학원 관리 프로그램 신규 개발. 현재 대성전산이 개발·운영 중인 Dlab통합관리프로그램(구 DSA)를 자체 시스템으로 대체한다. 재원생 앱과 **동일한 Spring Boot API 서버 + Supabase**를 공유한다.

### 1.2 운영 주체 및 관계사
| 주체 | 역할 | 비고 |
|---|---|---|
| ✅ **디랩(독학재수학원)** | **발주처(계약 상대)** | 계약·지급·운영 확정 항목(I-시리즈)·D-8 결정 권한 |
| 대성학력개발연구소(DSD) | D.Lab 사이트 운영, **앱 스토어 계정 보유** (관계사 — ~~발주처~~ 아님, v1.3 정정) | |
| 대성전산 | 현 Dlab통합관리프로그램(구 DSA)·**디멤버**·수납 프로그램 개발·운영 | DB 포함 전체 소유 |
| 자이엘코리아 | Zyxel 방화벽 장비 관리 | 김상원 팀장 / 010-6412-8131 |
| 급식업체 | 급식 공급·PG 가맹점(벤더 명의) | 연락처·환불 처리 |
| 개발팀(숀아이비엔엘) | 신규 Dlab통합관리프로그램(구 DSA)·앱 개발 | |
...
- **1:1 채팅**: 외부 유료 메신저 연동(React, ✅**서지원 — v1.3 담당 변경**, 7/10 "유현님"은 배치 확정 전 기준). `chat_threads`에 메타·참조ID만 저장, 본문은 벤더. ISMS 준용(E-6).
...
- 🆕 구글 시트 2개(I-15) 구조 실사 + 구 DSA 화면·데이터 실사(가능 범위).

### Phase 1 — MVP ✅(목표: 8월 ~ 9월 전반)
학생·대기자·입학예약 상담·출결(★D-2 키오스크 선결)·상벌점(★I-5 규칙 미수령 시 자동부여 제외)·문자/알림 발송(★알림톡 심사 선결)·관리자 기초설정(전년도 복사).

### Phase 2 — 핵심 ✅(목표: 9월 ~ 10월 전반)
급식(가상계좌·공휴일·취소 2트랙·데스크 당일 — ★E-3 PG 계약 선결)·수납현황(대성 API ★D-3·등록비 주체 ★D-9 확정 후)·교무업무(명단 엑셀)·특강·수납관리.

### Phase 3 — 교육·성적·소통 ✅(목표: 10월 ~ 11월)
성적(더프리미엄+상세 엑셀, 🖼️ 회차별·단원별 정답률)·**상담 리포트(🖼️ 별점·상담 항목 마스터·스티커)**·설문(+템플릿)·**데일리 루틴**·**주·일 학습 계획(🖼️ 교시 그리드·비율 파이·비교 그래프)**·**메시지(공지·행정요청·🖼️ 가족 채팅방)**·**1:1 채팅 연동(★E-6 메신저 선정 선결)**·승인 라우팅·**🖼️ Daily Report 집계·대면 질의응답·영단어 시험**.

### Phase 4 — 부가 ✅(목표: 12월 이후 — 앱 심사 대기 기간 병행 가능)
통계 대시보드·D.Lab 스토어(2027-12)·관리자 대시보드 고도화.
> 🆕 **11월**: 구 DSA 데이터 이관 리허설·통합 QA — 이진규 합류(P1 종료 후) 시점. **12월**: 앱 심사 대기 + 승인 후 사용자 QA + 컷오버 준비(🟡 2027년 신규 기수 개원 전 실사용 전환이 사실상 데드라인 — D-8과 연동 확정 필요).
...
| 10 | ~~2개월 납기~~ → ✅ 앱 11/30 개발완료·12월 심사(v1.3) | Phase 0·1 우선 원칙 유지. 잔여 리스크 = **12월 심사 반려 시 사용자 QA·컷오버 연쇄 지연** → 11월 중 TestFlight/내부테스트 선행으로 완충 |
| 11 | 🆕 **웹 FE 단일 인력 = 서지원(PM 겸임)** | 화면 물량 대비 병목. 화면 우선순위 강제(Phase순), 공통 컴포넌트 극대화, 위험 시 박유현 지원(P1·P2 부하 협의) 또는 외주 |
| 12 | 🆕 **BE 단일 인력 = 지유림(7~10월)** | 이진규 11월 합류 전까지 공유 백엔드 전담. 키오스크·PG·승인라우팅·규칙엔진 등 병렬 요구 → Phase 순서 엄수, 인터페이스 추상화로 후순위 스왑 |
